...
class ZabbixConfig(object):
    ...
    def __init__(self, module):
        ...
        try:
            ...
            self.session.headers.update({'Content-Type': 'application/json-rpc', 'Cache-Control': 'no-cache'})
            self.auth = self.do_request("user.login", {'user': self.user, 'password': self.password})
    ...
    def sort_api_args(self):
        ...
        if 'interfaces' in self.api_args:
            self.object['interfaces'] = sorted(self.object['interfaces'], key=lambda k: k['interfaceid'])
        if 'hosts' in self.api_args:
    ...
    def need_update(self, update, orig, is_recursive=False):
        ...
        for k in update.keys():
            if k not in orig:
                need_update = True
                continue
            if type(update[k]) != list and type(update[k]) != dict and update[k] != orig[k]:
                need_update = True
                continue
            if type(update[k]) != type(orig[k]):
                if (type(update[k]) != str and type(update[k]) != unicode) or \
                    (type(orig[k]) != str and type(orig[k]) != unicode) :
                    need_update = True
                    continue
            if type(orig[k]) == dict:
...
def main():
    ...
    fields = {
        "zabbix_url": {"required": True, "type": "str"},
        "zabbix_user": {"required": True, "type": "str"},
        "zabbix_password": {"required": True, "type": "str"},
        "api": {"required": True, "type": "str" },
        "api_uid": {"required": True, "type": "str" },
        "api_args": {"required": True, "type": "dict"},
        "state": {
            "default": "present",
    ...
    if (HAS_JSON is False):
        module.fail_json(msg="Missing required json module (check docs or install with: pip install requests)")
